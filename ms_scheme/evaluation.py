...
def _run_generic_benchmark(
    ...
    for r in range(rounds):
        ...
        partials = []
        for uk in user_keys:
            elapsed = _measure_ms(lambda k=uk: partials.append(sign_fn(mkeys, k, message, signer_ids, extra)))
...
def run_benchmark_ibms(n: int, q: int, signer_count: int, rounds: int = 30) -> BenchmarkRecord:
    return _run_generic_benchmark(
        ...
        sign_fn=lambda m, u, msg, sids, _: ibms_ntru.partial_sign(m, u, msg, sids),
        aggregate_fn=lambda ps, sids, _, qv: ibms_ntru.aggregate(ps, sids, qv),
        verify_fn=ibms_ntru.verify,
        prepare_fn=lambda _m, _s: None,
    )
...
def run_benchmark_cl(n: int, q: int, signer_count: int, rounds: int = 30) -> BenchmarkRecord:
    return _run_generic_benchmark(
        ...
        sign_fn=lambda m, u, msg, sids, _: cl_ntru_ms_irs.partial_sign(m, u, msg, sids),
        aggregate_fn=lambda ps, sids, _, qv: cl_ntru_ms_irs.aggregate(ps, sids, qv),
...
def run_benchmark_ni(n: int, q: int, signer_count: int, rounds: int = 30) -> BenchmarkRecord:
    return _run_generic_benchmark(
        ...
        rounds=rounds,
        poly_count=3,
        setup_fn=ni_ibms_pka.setup,
        extract_fn=ni_ibms_pka.extract_user_key,
        sign_fn=lambda m, u, msg, sids, agg_pk: ni_ibms_pka.partial_sign(m, u, msg, sids, agg_pk),
        aggregate_fn=lambda ps, sids, agg_pk, qv: ni_ibms_pka.aggregate(ps, sids, agg_pk, qv),
        verify_fn=ni_ibms_pka.verify,
        prepare_fn=lambda m, s: ni_ibms_pka.aggregate_public_keys(m.pp, s),
    )
...
def run_benchmark_csas(n: int, q: int, signer_count: int, rounds: int = 30) -> BenchmarkRecord:
    ...
    for r in range(rounds):
        ...
        user_keys = []
        for sid in signer_ids:
            elapsed = _measure_ms(lambda s=sid: user_keys.append(clsas_ntru.extract_user_key(mkeys, s)))
            key_extract_ms.append(elapsed)

        sig = clsas_ntru.init_signature(params.n)
        for uk in user_keys:
            holder = [sig]
            elapsed = _measure_ms(lambda k=uk: holder.__setitem__(0, clsas_ntru.sign_step(mkeys, k, message, holder[0])))
            partial_sign_ms.append(elapsed)
            sig = holder[0]

        aggregate_ms.append(0.0)
        verify_holder = [False]
