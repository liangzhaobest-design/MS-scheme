...
def extract_user_key(mkeys: CLMasterKeys, identity: str) -> CLUserKey:
    ...
    compact_sk = _poly_add(partial_sk, secret_value, p.q)

    public_key = _poly_mul_mod_xn1(_combined_base(mkeys.pp), compact_sk, p.n, p.q)
...
def verify(pp: CLPublicParams, message: bytes, signature: CLMultiSignature) -> bool:
    ...
    lhs = _poly_mul_mod_xn1(_combined_base(pp), signature.z, p.n, p.q)
    ...
    for sid in signature.signer_ids:
        ...
        compact_sk = _poly_add(partial_sk, secret_value, p.q)
        pk = _poly_mul_mod_xn1(_combined_base(pp), compact_sk, p.n, p.q)
